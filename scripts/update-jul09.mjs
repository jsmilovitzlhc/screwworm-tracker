...
const newCases = [
  {
    ...
    lng: -101.3978,
    status: 'active',
    notes: 'Tenth Crockett County case; cattle',
  },
];
...
try {
  ...
  await sql`
    INSERT INTO screwworm_metadata (key, value) VALUES ('lastUpdated', ${JSON.stringify('2026-07-09T00:00:00Z')})
    ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value
  `;
  await sql`
    INSERT INTO screwworm_metadata (key, value) VALUES ('lastChecked', ${JSON.stringify('2026-07-09T05:30:00Z')})
    ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value
  `;
  ...
  if (metaRows.length > 0) {
    ...
    const newEntry = {
      date: '2026-07-07',
      event: 'Case 33: cattle in Crockett County — tenth Crockett County case; US total reaches 33 with 23 active across 13 counties',
    };
  ...
  await sql`
    INSERT INTO screwworm_metadata (key, value) VALUES ('speciesBreakdown', ${JSON.stringify([
      { species: 'Bovine (Cattle)', count: 18, percentage: 55 },
      { species: 'Ovine (Sheep)', count: 10, percentage: 30 },
