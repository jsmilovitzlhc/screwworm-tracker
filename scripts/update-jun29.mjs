...
const newCase = {
  ...
  lng: -101.3878,
  status: 'active',
  notes: 'Sixth Crockett County case; another cattle case in Crockett County',
};
...
try {
  ...
  // Update status changes (cases #1, #3, #5 → inactive)
  ...
  await sql`
    INSERT INTO screwworm_metadata (key, value) VALUES ('lastUpdated', ${JSON.stringify('2026-06-29T00:00:00Z')})
    ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value
  `;
  await sql`
    INSERT INTO screwworm_metadata (key, value) VALUES ('lastChecked', ${JSON.stringify('2026-06-29T09:30:00Z')})
    ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value
  `;
  ...
  if (metaRows.length > 0) {
    ...
    const hasJun28 = (ctx.timeline || []).some(t => t.date === '2026-06-28');
    if (!hasJun28) {
      ctx.timeline.push({
        date: '2026-06-28',
